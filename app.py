...
def _generar_pdf(filas_tabla):
    ...
    with mpdf.PdfPages(buf) as pdf:
        ...
        ax.axis('off')
        for txt, y, fs, col in [
            ('Prediccion de CO₂ per Capita', 0.76, 22, '#4fc3f7'),
            ('Resolucion de Sistemas Lineales', 0.70, 15, 'white'),
            ('Metodos Numericos — Abril 2026', 0.64, 12, '#90caf9'),
            ('Dataset: Our World in Data CO2 (2019, 89 paises)', 0.58, 11, '#90caf9'),
            ('Tolerancia: 1e-6  |  Escenarios: Ideal / Estres / Mal Condicionado', 0.50, 10, '#81c995'),
            ('Metodos: Inversa | LU | Jacobi | Gauss-Seidel | SOR | GCP Sunagua (2020)', 0.44, 10, '#81c995'),
        ]:
            ax.text(0.5, y, txt, color=col, ha='center', va='center',
                    fontsize=fs, transform=ax.transAxes,
                    fontweight='bold' if y > 0.70 else 'normal')
        pdf.savefig(fig, facecolor='#0e1117')
        plt.close(fig)

        # ── Página 2: Tabla comparativa (landscape) ────────────────────
        ...
        # ── Página 3: Análisis e interpretación ────────────────────────
        fig = plt.figure(figsize=(8.27, 11.69))
        fig.patch.set_facecolor('#0e1117')
        ...
        parrafos = [
            ...
            ('Converge en exactamente 3 iteraciones en los 3 escenarios.\n'
             ...
             'El precondicionador C = diag(A)^⁻½ reduce el kappa efectivo.\n'
             'M = CCᵀ = diag(A)⁻¹ nunca se construye explicitamente (solo z = diag(A)*r).', 9, '#cccccc', False),
            ('', 6, 'white', False),
            ...
            ('Para n = 3 y matrices de tipo XᵀX, solo LU y GCP Sunagua son confiables.\n'
             ...
             'que x₃ (carbon) ≈ x₂ (energia) en los datos: el modelo no puede separar\n'
             'sus contribuciones marginales — fenomeno de multicolinealidad.\n\n'
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    **Propiedades de A = XᵀX:**
    ...
    st.markdown('---')
    st.subheader('Solucion exacta de referencia')
    st.markdown(
        'Solucion analitica del sistema: todos los metodos iterativos deben converger a estos pesos.'
    )
    st.latex(r'w^* = A^{-1}b')
    try:
        w_ref = np.linalg.solve(A, b)
        st.latex(r'w^* = ' + lmat(w_ref, fmt='.6f'))
        c1, c2, c3 = st.columns(3)
        c1.metric('w1 — PIB', f'{w_ref[0]:.6f}')
        c2.metric('w2 — Energia', f'{w_ref[1]:.6f}')
        c3.metric('w3 — Carbon', f'{w_ref[2]:.6f}')
        res_ref = float(np.linalg.norm(A @ w_ref - b))
        st.caption(f'Residual de la solucion exacta: ||Aw*−b|| = {res_ref:.2e}')
    except np.linalg.LinAlgError:
        st.error('A es singular — no existe solucion exacta unica (sistema mal condicionado extremo).')
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    **Por que GCP converge:**
    ...
    # Interpretacion en contexto CO2
    st.markdown('---')
    st.subheader('Interpretacion en el contexto de emisiones de CO2')
    st.markdown("""
- **Inversa y LU** calculan el peso exacto de cada variable (PIB, energia, carbon per capita)
  sobre las emisiones. Son los metodos de referencia.
- **Jacobi, Gauss-Seidel y SOR** divergen porque la matriz A = XᵀX de datos reales raramente
  tiene dominancia diagonal estricta: PIB y energia estan correlacionados, lo que hace que
  la suma de elementos fuera de la diagonal supere al elemento diagonal en algunas filas.
- **GCP Sunagua** converge exactamente en **3 iteraciones** en los 3 escenarios gracias a la
  garantia teorica para matrices SPD (A = XᵀX siempre lo es si X tiene rango completo).
  El precondicionador diagonal reduce el kappa efectivo sin construir M explicitamente.
- El **kappa alto** del escenario mal condicionado (x₃ ≈ x₂) refleja multicolinealidad:
  carbon y energia son casi la misma señal — el modelo no puede separar sus efectos marginales
  y la solucion se vuelve numericamente inestable para metodos directos de baja precision.
    """)
